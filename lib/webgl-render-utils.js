...
let unis = [
	...
	{ name: 'lightPos', type: '3fv', mat: false },
	{ name: 'lightC',   type: '3fv', mat: false },
...
function drawModel(gl, sp, model, opt) {
  ...
  let Pmat = mat4.create();
	...
	// Camera transform
	mat4.rotate(Mmat, -opt.camAngle[0] * Math.PI / 180, [0, 1, 0])
	mat4.rotate(Mmat, -opt.camAngle[1] * Math.PI / 180, [1, 0, 0])
	mat4.rotate(Mmat, -opt.camAngle[2] * Math.PI / 180, [0, 0, 1])
	mat4.translate(Mmat, [-opt.camPos[0], -opt.camPos[1], -opt.camPos[2]])

	// model tranform
	mat4.rotate(Mmat, model.angle * Math.PI / 180, [0, 1, 0])

	let uniValue = {}
	uniValue = Object.assign(uniValue, opt)
	uniValue.Pmat = Pmat
	uniValue.Mmat = Mmat

	// pass in the variables and data
	unis.forEach(uni => {
		passUni(gl, sp, uni.name, uni.type, uniValue[uni.name], uni.mat)
	})
